...
void GlobalRouter::dumpCapacities(std::string filename) {
    std::ofstream capFile(filename);
    if (!capFile.is_open()) {
        log() << "Error: cannot open file " << filename << std::endl;
        return;
    }
    // nLayers xSize ySize
    capFile << gridGraph.getNumLayers() << " " << gridGraph.getSize(0) << " " << gridGraph.getSize(1) << std::endl;
    // unit_length_wire_cost unit_via_cost unit_length_short_costs[0] unit_length_short_costs[1] unit_length_short_costs[2] ... ### unit_length_short_costs[i] represents the unit_length_short_cost for the i-th metall layer
    capFile << gridGraph.getUnitLengthWireCost() << " " << gridGraph.getUnitViaCost() << " ";
    for (int i = 0; i < gridGraph.getNumLayers(); i++) {
        capFile << gridGraph.getUnitLengthShortCost(i) << " ";
    } capFile << std::endl;
    // horizontal_GCell_edge_lengths[0] horizontal_GCell_edge_lengths[1] horizontal_GCell_edge_lengths[2] ... ### horizontal_GCell_edge_lengths[i] represents the length of the i-th horizontal GCell edge
    // vertical_GCell_edge_lengths[0] vertical_GCell_edge_lengths[1] vertical_GCell_edge_lengths[2] ... ### vertical_GCell_edge_lengths[i] represents the length of the i-th vertical GCell edge
    for (int i = 0; i < gridGraph.getSize(0); i++) {
        if (gridGraph.getEdgeLength(0, i) > 0)
            capFile << gridGraph.getEdgeLength(0, i) << " ";
    } capFile << std::endl;
    for (int i = 0; i < gridGraph.getSize(1); i++) {
        if (gridGraph.getEdgeLength(1, i) > 0)
            capFile << gridGraph.getEdgeLength(1, i) << " ";
    } capFile << std::endl;
    // for each layer:
    for (int layerIdx = 0; layerIdx < gridGraph.getNumLayers(); ++layerIdx) {
        auto dir = gridGraph.getLayerDirection(layerIdx);
        // layerNames[i] layerDirections[i] layerMinLengths[i] ### layerNames[i] represents the name of the i-th metal layer, layerDirections[i] represents the direction of the i-th metal layer, and layerMinLengths[i] represents the minimum length of a wire at the i-th metal layer.
        capFile << gridGraph.getLayerName(layerIdx) << " " << dir << " " << gridGraph.getLayerMinLength(layerIdx) << std::endl;
        // routing capacity for each GCell edge in layer i
        // ####
        for (int y = 0; y < gridGraph.getSize(1); y++) {
            for (int x = 0; x < gridGraph.getSize(0); x++) {
                capFile << gridGraph.getEdge(layerIdx, x, y).capacity;
                if (x != gridGraph.getSize(0)-1) capFile << " ";
            } capFile << std::endl;
        }
        // ####
    }
...
void GlobalRouter::dumpNets(std::string filename) {
    std::ofstream netFile(filename);
    for(const GRNet& net : nets) {
        netFile << net.getName() << std::endl;
        netFile << "(" << std::endl;
        for(const auto& gpts : net.getPinAccessPoints()) {
            netFile << "[";
            for(const auto& gpt : gpts) {
                netFile << "(" << gpt.layerIdx << ", " << gpt.x << ", " << gpt.y << ")";
                if (&gpt != &gpts.back()) netFile << ", ";
            }
            netFile << "]" << std::endl;
        }
        netFile << ")" << std::endl;
...
void GlobalRouter::route() {
    ...
    if(parameters.onlyMR){
        // dump net names of netIndices
        std::string fileName = parameters.design_name + ".ripup";
        std::ofstream file(fileName.c_str());
        for (const int netIndex : netIndices) {
            file << nets[netIndex].getName() << std::endl;
        }
        file.close();
        std::cout << "ripup nets: " << fileName << std::endl;
    }
...
void GlobalRouter::getGuides(const GRNet& net, vector<std::pair<std::pair<int,int>, utils::BoxT<int>>>& guides) { // guides: [<<l1,l2>, (x1,y1, x2,y2)>]
    ...
    GRTreeNode::preorder(routingTree, [&](std::shared_ptr<GRTreeNode> node) {
        for (const auto& child : node->children) {
            if (node->layerIdx == child->layerIdx) { // wire
                if (min(node->x, child->x)!=max(node->x, child->x) || min(node->y, child->y)!=max(node->y, child->y)){
                    guides.emplace_back(
                        std::pair<int,int>(node->layerIdx, node->layerIdx), utils::BoxT<int>(
                ...
                guides.emplace_back(std::pair<int,int>(min(node->layerIdx, child->layerIdx), maxLayerIndex), utils::BoxT<int>(node->x, node->y));
            }
        }
    });
    ...
    for (auto& gpts : net.getPinAccessPoints()) {
        for (auto& gpt : gpts) {
            if (gpt.layerIdx < parameters.min_routing_layer) {
                guides.emplace_back(std::pair<int,int>(gpt.layerIdx, parameters.min_routing_layer + 1), utils::BoxT<int>(gpt.x, gpt.y));
                areaOfPinPatches += (guides.back().second.x.range() + 1) * (guides.back().second.y.range() + 1) * ((parameters.min_routing_layer + 1) - gpt.layerIdx);
            }
        }
    }
