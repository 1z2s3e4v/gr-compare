...
parser.add_argument('-onlyMR', '--onlyMR', type=int, default=0)
...
def route():
    ...
    run('{cmd_prefix} ./{0} -lef {1}.input.lef -def {1}.input.def -threads {2} -onlyMR {3} -output {4} |& tee {5}'.format(
        binary, file_name_prefix, args.threads, args.onlyMR, guide_file, log_file, cmd_prefix=cmd_prefix))
...
for bm in bms:
    ...
    file_name_prefix = '{0}/iccad2019/{1}/{1}'.format(bm_path, bm.full_name)
