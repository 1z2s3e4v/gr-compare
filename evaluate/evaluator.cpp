    ...
    if(!net) {
      ...
      if(zh != zl) { // via
        if(xh == xl && yh == yl) {
          ...
          std::cerr << "Error occur at line " << line_count << "\n";
          NVR_ASSERT(0);
    ...
    for(unsigned x = 0; x < m_graph.num_gridx(); x++) {
      for(unsigned y = 0; y < m_graph.num_gridy(); y++) {
        ...
        if (cell.capacity() > 0.001 ){
          ...
          if(overflowLossFunc(overflow/2, overflow_slope) > 1)
          fprintf(fp, "(%d, %d, %d), cost: %lf, demand: %d, wire_counter: %d, stack via: %d, overflow: %lf, cap: %lf\n", z, x, y, overflowLossFunc(overflow/2, overflow_slope), demand, wire_counter[z][x][y], stacked_via_counter[z][x][y], overflow, cell.capacity());
        } else if (cell.capacity() >= 0 && cell.demand() > 0) {
          layer_overflows +=  overflowLossFunc(1.5 * double(cell.demand()), overflow_slope);
          
          if(overflowLossFunc(1.5 * double(cell.demand()), overflow_slope) > 1)
          fprintf(fp, "(%d, %d, %d), cost: %lf, demand: %d, wire_counter: %d, stack via: %d, overflow: %lf, cap: %lf\n", z, x, y, overflowLossFunc(1.5 * double(cell.demand()), overflow_slope), demand, wire_counter[z][x][y], stacked_via_counter[z][x][y], (double)demand, cell.capacity());
