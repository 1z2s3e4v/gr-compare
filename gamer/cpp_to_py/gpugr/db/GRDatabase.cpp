...
void GRDatabase::writeGuides(std::string outputFile) {
    ...
    auto singleGuide = [&](int xmin, int xmax, int ymin, int ymax, int lmin, int lmax) {
        number(xmin);
        s[cur++] = ' ';
        number(ymin);
        s[cur++] = ' ';
    ...
    auto printGrGuides = [&](int netId) {
        ...
        for (size_t i = 0; i < wires.size(); i += 2) {
            ...
            singleGuide(xmin, xmax, ymin, ymax, l, l);
        }
        auto vias = grNets[netId].getVias();
        for (auto p : vias) {
            int l = p / nMaxGrid / nMaxGrid, x = p % (nMaxGrid * nMaxGrid) / nMaxGrid, y = p % nMaxGrid;
            if (!(l & 1) ^ m1direction) std::swap(x, y);
            singleGuide(x, x, y, y, l, l+1);
            // if (l + 2 < nLayers) singleGuide(x, x, y, y, l + 1, l + 2);
        }
        auto pins = grNets[netId].getPins();
        for (auto& temp : pins) {
            int low = temp[0] / nMaxGrid / nMaxGrid, x = temp[0] % (nMaxGrid * nMaxGrid) / nMaxGrid, y = temp[0] % nMaxGrid;
            if (!(low & 1) ^ m1direction) std::swap(x, y);
            int high = temp[temp.size()-1] / nMaxGrid / nMaxGrid;
            int xmin = max(0, x - 1), xmax = min(xSize - 1, x + 1);
            int ymin = max(0, y - 1), ymax = min(ySize - 1, y + 1);
            for(int l=low; l<high; l++) {
                // if ((l & 1) ^ m1direction) {
                //     singleGuide(x, x, ymin, ymax, l, l);
                // } else {
                //     singleGuide(xmin, xmax, y, y, l, l);
                // }
                if (l+1 < nLayers) {
                    singleGuide(x, x, y, y, l, l+1);
                }
