...
void GuideGenerator::genConnGuides() {
    // keep connectivity
    int minLayer = database.getLayerNum();
    int maxLayer = 0;
    for (auto &point : grNet.ovlpPoints) {
        if (point.layerIdx < minLayer) minLayer = point.layerIdx;
        if (point.layerIdx > maxLayer) maxLayer = point.layerIdx;
    ...
    if (minLayer != maxLayer){
        for (auto &point : grNet.ovlpPoints) {
            grNet.viaRouteGuides1.emplace_back(minLayer,
                                            utils::IntervalT<int>(point[X], point[X]),
                                            utils::IntervalT<int>(point[Y], point[Y]));
            grNet.viaRouteGuides2.emplace_back(maxLayer,
                                            utils::IntervalT<int>(point[X], point[X]),
                                            utils::IntervalT<int>(point[Y], point[Y]));
            break;
        }
    ...
    for (auto &pbxs : grNet.pinAccessBoxes) {
        for (auto &pbx : pbxs) {
            ...
            if (pbx.layerIdx < database.getLayerNum() - 2) {
                ...
                auto bxPlus0 = gr::GrPoint(pbx.layerIdx, pbx.x, pbx.y);
                ...
                if (needPatch(bxPlus1) || needPatch(bxPlus2)) {
                    ...
                    // for ispd24 guide
                    grNet.patchRouteGuides1.emplace_back(getSurrounding(bxPlus0));
                    grNet.patchRouteGuides2.emplace_back(getSurrounding(bxPlus2));
                } else {
                    ...
                    // for ispd24 guide
                    grNet.patchRouteGuides1.emplace_back(gr::GrBoxOnLayer(bxPlus0.layerIdx, {pbx.x}, {pbx.y}));
                    grNet.patchRouteGuides2.emplace_back(gr::GrBoxOnLayer(bxPlus2.layerIdx, {pbx.x}, {pbx.y}));
                }
            }
            // patch lower two layers
            if (pbx.layerIdx > 1) {
                guideGenStat.pinRegionPatchCand++;
                auto bxMinus0 = gr::GrPoint(pbx.layerIdx, pbx.x, pbx.y);
                ...
                if (needPatch(bxMinus1) || needPatch(bxMinus2)) {
                    ...
                    // for ispd24 guide
                    grNet.patchRouteGuides1.emplace_back(getSurrounding(bxMinus2));
                    grNet.patchRouteGuides2.emplace_back(getSurrounding(bxMinus0));
                } else {
                    ...
                    // for ispd24 guide
                    grNet.patchRouteGuides1.emplace_back(gr::GrBoxOnLayer(bxMinus2.layerIdx, {pbx.x}, {pbx.y}));
                    grNet.patchRouteGuides2.emplace_back(gr::GrBoxOnLayer(bxMinus0.layerIdx, {pbx.x}, {pbx.y}));
                }
            }
    ...
    for (const auto &guide : grNet.wireRouteGuides) {
        ...
        for (int cp = guide[1 - dir].low + offset; cp <= guide[1 - dir].high;) {
            ...
            if (grDatabase.getCellUsage({layerIdx, x, y}) < patchThresh) {
                bool patched = false;
                int pathLayerMin = layerIdx;
                int pathLayerMax = layerIdx;
                for (int layer_delta = -1; layer_delta <= 1; layer_delta += 2) {
                    ...
                    if (grDatabase.getCellUsage({layer, x, y}) > patchThresh) {
                        ...
                        patched = true;
                        if (layer < pathLayerMin) pathLayerMin = layer;
                        if (layer > pathLayerMax) pathLayerMax = layer;
                    }
                }
                if (patched) {
                    cp += patchIntvl;
                    // for ispd24 guide 
                    grNet.patchRouteGuides1.emplace_back(gr::GrBoxOnLayer(pathLayerMin, {x}, {y}));
                    grNet.patchRouteGuides2.emplace_back(gr::GrBoxOnLayer(pathLayerMax, {x}, {y}));
                } else {
                    cp++;
                }
            } else {
                ...
                if (cellUsage > 0) {
                    ...
                    for (auto l : layers) {
                        ...
                        if (vioFree) {
                            ...
                            // for ispd24 guide
                            if (l != patch.layerIdx){
                                grNet.patchRouteGuides1.emplace_back(min(l,patch.layerIdx), patch[X], patch[Y]);
                                grNet.patchRouteGuides2.emplace_back(max(l,patch.layerIdx), patch[X], patch[Y]);
                            }
                            break;
                        }
                    }
...
void GuideGenerator::genPatchGuides() {
    ...
    grNet.patchRouteGuides1.clear();
    grNet.patchRouteGuides2.clear();
    patchPinRegions();
    patchLongSegments();
    patchVioCells();
}
...
void GuideGenerator::genTopoGuides() {
    ...
    grNet.viaRouteGuides1.clear();
    grNet.viaRouteGuides2.clear();
    ...
    grNet.postOrderVisitGridTopo([&](std::shared_ptr<gr::GrSteiner> node) {
        ...
        for (auto child : parent->children) {
            if (parent->layerIdx == child->layerIdx) {
                ...
                // for ispd24 guide                                                  
                grNet.viaRouteGuides1.emplace_back(min(parent->layerIdx, child->layerIdx),
                                                  utils::IntervalT<int>((*parent)[X], (*parent)[X]),
                                                  utils::IntervalT<int>((*parent)[Y], (*parent)[Y]));
                grNet.viaRouteGuides2.emplace_back(max(parent->layerIdx, child->layerIdx),
                                                  utils::IntervalT<int>((*parent)[X], (*parent)[X]),
                                                  utils::IntervalT<int>((*parent)[Y], (*parent)[Y]));
            }
        }
    ...
    sliceGuides(grNet.viaRouteGuides1);
    sliceGuides(grNet.viaRouteGuides2);
}
