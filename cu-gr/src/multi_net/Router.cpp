...
vector<int> Router::getNetsToRoute() {
    ...
    if (iter == 0) {
        ...
        if (onlyMR) { // read ripuped nets in ripup_file and reroute them
            std::string design_name = db::setting.name;
            size_t pos = design_name.find_last_of('/');
            design_name = design_name.substr(pos + 1);
            std::string ripup_file = design_name + ".ripup";
            std::cout << "Read ripuped nets in " << ripup_file << std::endl;
            std::ifstream fin(ripup_file);
            std::vector<std::string> ripupNetNames;
            while (!fin.eof()) {
                std::string netName;
                fin >> netName;
                ripupNetNames.push_back(netName);
            }
            for (auto& net : grDatabase.nets) {
                if(std::find(ripupNetNames.begin(), ripupNetNames.end(), net.getName()) != ripupNetNames.end()){
