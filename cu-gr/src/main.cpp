...
void runISPD18Flow(const boost::program_options::variables_map& vm) {
    ...
    if (vm.count("transFormatToISPD24")) {
        db::setting.transFormatToISPD24 = vm.at("transFormatToISPD24").as<bool>();
    }
    if (vm.count("onlyMR")) {
        db::setting.onlyMR = vm.at("onlyMR").as<int>();
    }
    // multi_net
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        desc.add_options()
                ...
                // optional
                ("transFormatToISPD24", value<bool>(), "lefdef to capnet (ispd24 format)")
                ("onlyMR", value<int>()->required(), "onlyMR")
